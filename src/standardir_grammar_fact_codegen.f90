module standardir_grammar_fact_codegen
    ...
    public :: standardir_generate_grammar_fact_table
...
contains
    ...
    subroutine standardir_generate_grammar_fact_table(nodes, unit, ok, message)
        type(sx_node_t), intent(in) :: nodes(:)
        integer, intent(in) :: unit
        logical, intent(out) :: ok
        character(len=*), intent(out) :: message
        ...
        if (size(nodes) == 0) then
            message = 'grammar-fact table has no source-backed facts'
            return
        end if
        allocate (facts(size(nodes)))
        do i = 1, size(nodes)
        ...
        call emit_grammar_fact_table(unit, facts)
        ok = .true.
    end subroutine standardir_generate_grammar_fact_table

    subroutine emit_grammar_fact_table(unit, facts)
        integer, intent(in) :: unit
        ...
        call emit_line(unit, 'module standardir_grammar_fact_table_generated')
        ...
        call emit_line(unit, '')
        call emit_line(unit, '    type, public :: standardir_grammar_fact_table_entry_t')
        call emit_line(unit, '        type(grammar_fact_t) :: fact')
        call emit_line(unit, '    end type standardir_grammar_fact_table_entry_t')
        call emit_line(unit, '')
        call emit_line(unit, '    integer, parameter, public :: standardir_grammar_fact_table_count = '// &
            trim(count_text))
        call emit_line(unit, '    integer, parameter, public :: STANDARDIR_GRAMMAR_FACT_COLLECTION_SUCCESS = 0')
        call emit_line(unit, '    integer, parameter, public :: STANDARDIR_GRAMMAR_FACT_COLLECTION_MISSING = 1')
        call emit_line(unit, '    integer, parameter, public :: STANDARDIR_GRAMMAR_FACT_COLLECTION_MALFORMED = 2')
        call emit_line(unit, '    integer, parameter, public :: STANDARDIR_GRAMMAR_FACT_COLLECTION_CAPACITY = 3')
        call emit_line(unit, '    public :: standardir_make_grammar_fact_table')
        call emit_line(unit, '    public :: standardir_lookup_grammar_fact')
        call emit_line(unit, '    public :: standardir_collect_grammar_facts')
        call emit_line(unit, '')
        call emit_line(unit, 'contains')
        call emit_line(unit, '')
        call emit_line(unit, '    subroutine standardir_make_grammar_fact_table(values)')
        call emit_line(unit, '        type(standardir_grammar_fact_table_entry_t), intent(out) :: values('// &
            trim(count_text)//')')
        call emit_line(unit, '')
        do i = 1, size(facts)
            call emit_fact_assignment(unit, i, facts(i))
        end do
        call emit_line(unit, '    end subroutine standardir_make_grammar_fact_table')
        call emit_line(unit, '')
        call emit_line(unit, '    subroutine standardir_lookup_grammar_fact(id, expression, value, found)')
        call emit_line(unit, '        character(len=*), intent(in) :: id, expression')
        call emit_line(unit, '        type(standardir_grammar_fact_table_entry_t), intent(out) :: value')
        call emit_line(unit, '        logical, intent(out) :: found')
        call emit_line(unit, '')
        call emit_line(unit, '        type(standardir_grammar_fact_table_entry_t) :: values('//trim(count_text)//')')
        call emit_line(unit, '        integer :: i')
        call emit_line(unit, '')
        call emit_line(unit, '        call standardir_make_grammar_fact_table(values)')
        ...
        call emit_line(unit, '    end subroutine standardir_lookup_grammar_fact')
        call emit_line(unit, '')
        call emit_line(unit, '    subroutine standardir_collect_grammar_facts(id, output, count, status)')
        call emit_line(unit, '        character(len=*), intent(in) :: id')
        call emit_line(unit, '        type(standardir_grammar_fact_table_entry_t), intent(out) :: output(:)')
        call emit_line(unit, '        integer, intent(out) :: count, status')
        call emit_line(unit, '')
        call emit_line(unit, '        type(standardir_grammar_fact_table_entry_t) :: values('//trim(count_text)//')')
        call emit_line(unit, '        integer :: i, matches')
        call emit_line(unit, '')
        call emit_line(unit, '        count = 0')
        call emit_line(unit, '        status = STANDARDIR_GRAMMAR_FACT_COLLECTION_MALFORMED')
        call emit_line(unit, '        if (.not. standardir_valid_grammar_fact_rule_id(id)) then')
        call emit_line(unit, '            call standardir_clear_grammar_fact_output(output)')
        call emit_line(unit, '            return')
        call emit_line(unit, '        end if')
        call emit_line(unit, '')
        call emit_line(unit, '        call standardir_make_grammar_fact_table(values)')
        call emit_line(unit, '        matches = 0')
        call emit_line(unit, '        do i = 1, size(values)')
        call emit_line(unit, '            if (trim(values(i)%fact%id) == trim(id)) matches = matches + 1')
        call emit_line(unit, '        end do')
        call emit_line(unit, '        if (matches == 0) then')
        call emit_line(unit, '            status = STANDARDIR_GRAMMAR_FACT_COLLECTION_MISSING')
        call emit_line(unit, '            call standardir_clear_grammar_fact_output(output)')
        call emit_line(unit, '            return')
        call emit_line(unit, '        end if')
        call emit_line(unit, '        if (matches > size(output)) then')
        call emit_line(unit, '            status = STANDARDIR_GRAMMAR_FACT_COLLECTION_CAPACITY')
        call emit_line(unit, '            call standardir_clear_grammar_fact_output(output)')
        call emit_line(unit, '            return')
        call emit_line(unit, '        end if')
        call emit_line(unit, '        do i = 1, size(values)')
        call emit_line(unit, '            if (trim(values(i)%fact%id) == trim(id)) then')
        call emit_line(unit, '                count = count + 1')
        call emit_line(unit, '                output(count) = values(i)')
        call emit_line(unit, '            end if')
        call emit_line(unit, '        end do')
        call emit_line(unit, '        status = STANDARDIR_GRAMMAR_FACT_COLLECTION_SUCCESS')
        call emit_line(unit, '    end subroutine standardir_collect_grammar_facts')
        call emit_line(unit, '')
        call emit_line(unit, '    logical function standardir_valid_grammar_fact_rule_id(id)')
        call emit_line(unit, '        character(len=*), intent(in) :: id')
        call emit_line(unit, '        integer :: i, code')
        call emit_line(unit, '')
        call emit_line(unit, '        standardir_valid_grammar_fact_rule_id = .false.')
        call emit_line(unit, '        if (len_trim(id) < 2) return')
        call emit_line(unit, '        if (id(1:1) /= ''R'') return')
        call emit_line(unit, '        do i = 2, len_trim(id)')
        call emit_line(unit, '            code = iachar(id(i:i))')
        call emit_line(unit, '            if (code < iachar(''0'') .or. code > iachar(''9'')) return')
        call emit_line(unit, '        end do')
        call emit_line(unit, '        standardir_valid_grammar_fact_rule_id = .true.')
        call emit_line(unit, '    end function standardir_valid_grammar_fact_rule_id')
        call emit_line(unit, '')
        call emit_line(unit, '    subroutine standardir_clear_grammar_fact_output(output)')
        call emit_line(unit, '        type(standardir_grammar_fact_table_entry_t), intent(out) :: output(:)')
        call emit_line(unit, '        integer :: i')
        call emit_line(unit, '')
        call emit_line(unit, '        do i = 1, size(output)')
        call emit_line(unit, '            output(i)%fact%id = ''''')
        call emit_line(unit, '            output(i)%fact%expression = ''''')
        call emit_line(unit, '            output(i)%fact%source%document = ''''')
        call emit_line(unit, '            output(i)%fact%source%clause = ''''')
        call emit_line(unit, '            output(i)%fact%source%rule = ''''')
        call emit_line(unit, '            output(i)%fact%source%page = 0')
        call emit_line(unit, '            output(i)%fact%source%source_hash = ''''')
        call emit_line(unit, '            output(i)%fact%origin = 0')
        call emit_line(unit, '            output(i)%fact%resolution = 0')
        call emit_line(unit, '        end do')
        call emit_line(unit, '    end subroutine standardir_clear_grammar_fact_output')
        call emit_line(unit, '')
        call emit_line(unit, 'end module standardir_grammar_fact_table_generated')
    end subroutine emit_grammar_fact_table
