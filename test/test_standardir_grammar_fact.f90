program test_standardir_grammar_fact
    ...
    use standardir_grammar_fact_codegen, only: standardir_generate_integer_type_spec_fact, &
        ...
        standardir_generate_add_op_en_dash_grammar_fact, &
    ...
    use standardir_add_op_en_dash_grammar_fact, only: &
        standardir_consume_add_op_en_dash_grammar_fact, standardir_write_add_op_en_dash_grammar_fact
    ...
    character(len=*), parameter :: expected_add_op = &
        ...
        '(page 155) (source-hash '//source_hash//'))) (origin mechanical) (resolution resolved))'
    character(len=*), parameter :: expected_add_op_en_dash = &
        '(grammar-fact (id R1010) (expression "–") '// &
        '(source (source-ref (document J3-24-007) (clause 10) (rule R1010) '// &
        '(page 155) (source-hash '//source_hash//'))) (origin mechanical) (resolution resolved))'
    ...
    call check_expression_fact(expected_add_op_en_dash, 'R1010', '–', &
        standardir_write_add_op_en_dash_grammar_fact, standardir_consume_add_op_en_dash_grammar_fact, &
        standardir_generate_add_op_en_dash_grammar_fact)
...
contains
    ...
    subroutine check_expression_generated_source_freshness()
        ...
        open (newunit=input_unit, file='specs/grammar-facts-v0.sx', action='read', iostat=ios)
        call require(ios == 0, 'could not reopen add-op en-dash grammar-fact specification')
        do i = 1, 14
            read (input_unit, '(a)', iostat=ios) source
            call require(ios == 0, 'could not read add-op en-dash grammar-fact specification')
        end do
        close (input_unit)
        call sx_parse(trim(source), node, local_ok, message)
        call require(local_ok, message)
        open (newunit=output_unit, file='build/standardir_add_op_en_dash_grammar_fact_generated.f90', &
            status='replace', action='write', iostat=ios)
        call require(ios == 0, 'could not open fresh add-op en-dash output')
        call standardir_generate_add_op_en_dash_grammar_fact(node, output_unit, local_ok, message)
        close (output_unit)
        call require(local_ok, message)
        call read_source('build/standardir_add_op_en_dash_grammar_fact_generated.f90', fresh, fresh_count)
        call read_source('src/standardir_add_op_en_dash_grammar_fact_generated.f90', checked, checked_count)
        call require(fresh_count == checked_count .and. all(fresh(:fresh_count) == checked(:checked_count)), &
            'checked-in add-op en-dash output differs from specification')
