program sxtreesitter
    ...
    if (.not. ok) call fail(trim(message))
    do i = 1, group_count
        call standardir_emit_treesitter_group(output_unit, nodes, groups(i), ok, message)
        if (.not. ok) call fail(trim(message))
    end do
    write (output_unit, '(a)') '  }'
    write (output_unit, '(a)') '});'
    close (input_unit)
    close (output_unit)
    print '(a,i0,a,i0,a)', 'emitted ', group_count, ' tree-sitter productions from ', records, ' records'
